...
def load_data(file_index, n):
    ...
    if args.single_frame_initialization:
        train_x = fields_u[:n, ::sub, ::sub, 0:1].repeat(1, 1, 1, T_in)
        train_y = fields_u[:n, ::sub, ::sub, 1:1 + T_rollout]
    else:
        train_x = fields_u[:n, ::sub, ::sub, :T_in]
        train_y = fields_u[:n, ::sub, ::sub, T_in:T_in + T_rollout]
    
    train_x = train_x.reshape(n, S//sub, S//sub, T_in, 1)
    train_y = train_y.reshape(n, S//sub, S//sub, T_rollout, 1)
