"""
lucidrains rope implementation. Meant to eventually be replaced,
but clearly never got to it.
"""
...
class RotaryEmbedding(Module):
    ...
    def make_learnable(self, per_axis=False):
        """
        Make the RoPE learnable.
        """
        if per_axis:
            self.per_axis_freqs = True
            self.freqs = torch.nn.ParameterList(
                [torch.nn.Parameter(self.freqs.clone()) for _ in range(3)]
            )
            print("Using per-axis learnable RoPE")
        else:
            self.freqs = torch.nn.Parameter(self.freqs)
            print("Using single learnable RoPE")
    ...
    def get_scale(self, t: Tensor, seq_len: int | None = None, offset=0):
        ...
        if self.use_xpos:
            ...
            scale: Tensor = self.scale ** rearrange(power, "n -> n 1")
    ...
    @autocast("cuda", enabled=False)
    def forward(self, t: Tensor, seq_len=None, offset=0, index=0):
        should_cache = (
            self.cache_if_possible
