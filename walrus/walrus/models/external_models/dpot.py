# ruff: noqa
"""
Copied directly from the upstream DPOT implementation.
"""
import logging
...
class Mlp(nn.Module):
    def __init__(
        self, in_features, hidden_features=None, out_features=None, act="gelu", drop=0.0
    ):
        super().__init__()
...
class DPOTNet(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(
                1, embed_dim, self.patch_embed.out_size[0], self.patch_embed.out_size[1]
            )
        )
        ...
        self.time_agg_layer = TimeAggregator(
            in_channels, in_timesteps, embed_dim, time_agg
        )
