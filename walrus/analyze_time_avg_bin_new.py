...
def noflip_nrmse(pred, ref):
    return nrmse(pred, ref)


def find_flow_h5(case_dir):
    candidates = sorted(
        f for f in os.listdir(case_dir)
        if f.endswith('.h5') and f.startswith('flow')
    )
    if not candidates:
        return None
    if len(candidates) > 1:
        print(f"WARNING: multiple flow*.h5 in {case_dir}, using {candidates[0]}")
    return os.path.join(case_dir, candidates[0])
...
EXPECTED_T = 201
LAST_N = 160                                # time-averaged metrics use last 160 timesteps
INST_SLICE = slice(0, 40)                   # instantaneous metrics use first 40 timesteps
...
for seed in seed_dirs:
    ...
    for d in os.listdir(seed_path):
        case_dir = os.path.join(seed_path, d)
        if not os.path.isdir(case_dir):
            continue
        h5path = find_flow_h5(case_dir)
        if h5path is None:
            continue
...
ref_by_seed = {}
for seed in seed_dirs:
    ref_case_dir = os.path.join(basedir, seed, baseres)
    if not os.path.isdir(ref_case_dir):
        print(f'WARNING: no reference ({baseres}) for seed {seed}, skipping seed')
        continue
    ref_path = find_flow_h5(ref_case_dir)
    if ref_path is None:
        print(f'WARNING: no flow*.h5 in reference ({baseres}) for seed {seed}, skipping seed')
        continue
...
noflip_cols = [f'noflip_{q}' for q in print_quantities]
all_cols = tavg_cols + inst_cols + noflip_cols
...
for cs in all_coarse:
    ...
    seed_noflip_nrmse = defaultdict(list)
    wallclocks = []
    ...
    for seed in active_seeds:
        ...
        if not matches:
            continue
        dname = matches[0]
        h5path = find_flow_h5(os.path.join(basedir, seed, dname))
        if h5path is None:
            continue
        ...
        for q in print_quantities:
            seed_noflip_nrmse[q].append(noflip_nrmse(data[q], ref[q]))
    ...
    for q in print_quantities:
        row += f"  {np.mean(seed_noflip_nrmse[q]):>20.6f}"
