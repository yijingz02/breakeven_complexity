...
for NDATA in "${ndatas[@]}"; do
  python train.py \
    ...
    optimizer=adam optimizer.lr=1e-3 logger.wandb_project_name="walrus_bf" \
    trainer.enable_amp=True model.gradient_checkpointing_freq=1 trainer.log_interval=1000 trainer.clip_gradient=1 \
    data.module_parameters.batch_size=8 \
    data.module_parameters.inference_batch_size=8 \
    data.module_parameters.n_steps_input=10 \
    data.module_parameters.n_steps_output=1 \
    data.module_parameters.well_dataset_info.breakflow.path="${DATA_PATH}" \
    ...
    trainer.prediction_type=delta data=breakflow trainer.max_epoch=10000 data_workers=10 model.override_dimensionality=0 auto_resume=True \
