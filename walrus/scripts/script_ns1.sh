...
for NDATA in "${ndatas[@]}"; do
  python train.py \
    ...
    optimizer=adam optimizer.lr=1e-3 logger.wandb_project_name="walrus_ns" \
    ...
    data.module_parameters.well_dataset_info.ns_exponax.path="${DATA_PATH}" \
    ...
    trainer.prediction_type=delta data=ns_exponax trainer.max_epoch=10000 data_workers=10 model.override_dimensionality=0 auto_resume=True \
    ...
    ++data.module_parameters.start_rollout_valid_output_at_t=2 \
    ...
    ++trainer.gen_time_s=0.04353754370212555 \
