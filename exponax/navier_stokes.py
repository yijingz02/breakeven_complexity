...
parser.add_argument(
    ...
    type=float,
    default=2,
    help="Simulation spatial size. Will be multipled by pi."
)
parser.add_argument(
    "--batch_size",
    type=int,
...
for i in range(args.nfile):
    ...
    while data_cnt < per_file:
        # print(f"data: {data_cnt}")
        ts = time.time()
        file_key, u_trj, _u0 = generate_batch(file_key, ns_stepper, args.batch_size, steps, save_step)
...
while data_cnt < args.val_size:
    val_key, u_trj, u_0 = generate_batch(val_key, ns_stepper, args.batch_size, steps, save_step)
    u_trj = spectral_downsample(u_trj, 64)
