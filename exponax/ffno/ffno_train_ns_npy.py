...
parser = argparse.ArgumentParser(description="NS FFNO Trainer")
parser.add_argument(
    "--save",
    type=str,
...
per_step_time = 0.41
per_data_generation_time = 0.04353754370212555
...
def load_data(file_index, n):
    ...
    if file_index == "val":
        train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(train_x, train_y), batch_size=10, shuffle=True)
    else:
...
output_path = f"ffno_NS_NvsB_budget{int(time_budget)}_n{ntrain}_w{width}_m{modes}_batch{batch_size}"
