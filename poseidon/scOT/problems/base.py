...
def get_dataset(dataset, **kwargs):
    ...
    **kwargs overwrite the default settings.
    ...
    if "fluids" in dataset:
        ...
        elif "fluids.MultiObstacleIncompressible" in dataset:
            from .fluids.multi_obstacle import MultiObstacleIncompressible as dset
            default_time_settings = {"max_num_time_steps": 21, "time_step_size": 1}
            kwargs = {**default_time_settings, **kwargs}
        elif "fluids.NavierStokesMultiFileMultiFrame" in dataset:
            from .fluids.navier_stokes import NavierStokesMultiFileMultiFrame as dset
            default_time_settings = {"max_num_time_steps": 10, "time_step_size": 1}
            kwargs = {**default_time_settings, **kwargs}
        elif "fluids.MultiObstacleVorticityMultiFrame" in dataset:
            from .fluids.multi_obstacle import MultiObstacleVorticityMultiFrame as dset
            default_time_settings = {"max_num_time_steps": 21, "time_step_size": 1}
            kwargs = {**default_time_settings, **kwargs}
        elif "fluids.KuramotoSivashinskyMultiFileMultiFrame" in dataset:
            from .fluids.kuramoto_sivashinsky import KuramotoSivashinskyMultiFileMultiFrame as dset
            default_time_settings = {"max_num_time_steps": 50, "time_step_size": 1}
            kwargs = {**default_time_settings, **kwargs}
        elif "fluids.NavierStokesMultiFile" in dataset:
            from .fluids.navier_stokes import NavierStokesMultiFile as dset
            default_time_settings = {"max_num_time_steps": 10, "time_step_size": 1}
            kwargs = {**default_time_settings, **kwargs}
        elif "fluids.MultiObstacleVorticity" in dataset:
            from .fluids.multi_obstacle import MultiObstacleVorticity as dset
            default_time_settings = {"max_num_time_steps": 21, "time_step_size": 1}
            kwargs = {**default_time_settings, **kwargs}
        elif "fluids.KuramotoSivashinskyMultiFile" in dataset:
            from .fluids.kuramoto_sivashinsky import KuramotoSivashinskyMultiFile as dset
            default_time_settings = {"max_num_time_steps": 50, "time_step_size": 1}
            kwargs = {**default_time_settings, **kwargs}
        else:
        ...
        elif "reaction_diffusion.GrayScottMultiFile" in dataset:
            if "out" in dataset:
                default_time_settings = {"max_num_time_steps": 200, "time_step_size": 1}
            else:
                default_time_settings = {"max_num_time_steps": 200, "time_step_size": 1}
            kwargs = {**default_time_settings, **kwargs}
            from .reaction_diffusion.gray_scott import GrayScottMultiFile as dset
    else:
        raise ValueError(f"Unknown dataset {dataset}")
