...
class MixedTemporalDataset(Dataset):
    ...
    def __init__(self, data_names, n_list = None, res = 128,t_in = 10, t_ar = 1, n_channels = None, normalize=False,train=True,data_weights=None):
        ...
        for name in self.data_names:
            data_cfg = DATASET_DICT[name]
            storage_format = data_cfg.get('storage_format', 'hdf5')
            if storage_format == 'npy':
                npy_storage = self._build_npy_storage(data_cfg, train=train)
                self.data_files.append(npy_storage)
                self.storage_formats.append('npy')
                self.max_n_sizes.append(npy_storage['total_size'])
            elif DATASET_DICT[name]['scatter_storage']:
                ...
                self.storage_formats.append('hdf5_scatter')
                self.max_n_sizes.append(DATASET_DICT[name]['train_size'] if train else DATASET_DICT[name]['test_size'])
                ...
                self.storage_formats.append('hdf5')
                self.max_n_sizes.append(DATASET_DICT[name]['train_size'] if train else DATASET_DICT[name]['test_size'])
        ...
        for dataset_name, requested_size, max_size in zip(self.data_names, self.n_sizes, self.max_n_sizes):
            if requested_size > max_size:
                raise ValueError(f"Requested size {requested_size} for dataset {dataset_name} exceeds available samples {max_size}")
        ...
        if normalize:
            ...
            for storage_format, data in zip(self.storage_formats, self.data_files):
                if storage_format == 'npy':
                    print('Warning: normalize=True is currently not supported for npy storage; skip per-dataset normalization.')
    ...
    @staticmethod
    def _resolve_npy_paths(base_path, file_entries):
        resolved_paths = []
        for file_entry in file_entries:
            candidate = os.path.join(base_path, file_entry)
            ...
            if matched:
                ...
                raise FileNotFoundError(f'Cannot find npy file pattern or path: {candidate}')
        if len(resolved_paths) == 0:
            raise FileNotFoundError(f'No npy files found under {base_path} from entries: {file_entries}')
        return resolved_paths

    def _build_npy_storage(self, data_cfg, train=True):
        base_path = data_cfg['train_path'] if train else data_cfg['test_path']
        files_cfg = data_cfg['train_files'] if train else data_cfg['test_files']
        channel_keys = data_cfg.get('channel_keys', list(files_cfg.keys()))

        channel_arrays = {}
        for channel_key in channel_keys:
            if channel_key not in files_cfg:
                raise KeyError(f'Channel {channel_key} is missing in npy files config for dataset.')
            channel_paths = self._resolve_npy_paths(base_path, files_cfg[channel_key])
            channel_arrays[channel_key] = [np.load(path, mmap_mode='r') for path in channel_paths]

        n_files = len(channel_arrays[channel_keys[0]])
        for channel_key in channel_keys:
            if len(channel_arrays[channel_key]) != n_files:
                raise ValueError('All channels in npy dataset must have the same number of files.')

        file_lengths = []
        for file_idx in range(n_files):
            reference_shape = channel_arrays[channel_keys[0]][file_idx].shape
            if len(reference_shape) != 4:
                raise ValueError(f'Expected npy array shape (N, H, W, T), got {reference_shape}')
            for channel_key in channel_keys[1:]:
                channel_shape = channel_arrays[channel_key][file_idx].shape
                if channel_shape != reference_shape:
                    raise ValueError(f'All channels must share the same shape. Got {reference_shape} and {channel_shape}')
            file_lengths.append(reference_shape[0])

        cumulative_sizes = np.cumsum(file_lengths)
        
        # Load obstacle mask if provided
        obstacle_masks = None
        mask_files_key = 'train_mask_files' if train else 'test_mask_files'
        if mask_files_key in data_cfg:
            mask_paths = self._resolve_npy_paths(base_path, data_cfg[mask_files_key])
            obstacle_masks = [np.load(path, mmap_mode='r') for path in mask_paths]
    ...
    @staticmethod
    def _load_npy_sample(npy_storage, data_idx):
        file_idx = int(np.searchsorted(npy_storage['cumulative_sizes'], data_idx + 1))
        sample_start = 0 if file_idx == 0 else int(npy_storage['cumulative_sizes'][file_idx - 1])
        local_idx = int(data_idx - sample_start)
        sample_channels = [
            np.asarray(npy_storage['channel_arrays'][channel_key][file_idx][local_idx], dtype=np.float32)
            for channel_key in npy_storage['channel_keys']
        ]
        return np.stack(sample_channels, axis=-1)

    @staticmethod
    def _load_npy_obstacle_mask(npy_storage, data_idx):
        """Load a per-sample obstacle mask from an npy storage.

        Expected mask file shape:
        - (N, H, W)
        """
        if npy_storage['obstacle_masks'] is None:
            return None
        
        file_idx = int(np.searchsorted(npy_storage['cumulative_sizes'], data_idx + 1))
        sample_start = 0 if file_idx == 0 else int(npy_storage['cumulative_sizes'][file_idx - 1])
        local_idx = int(data_idx - sample_start)
        mask_array = npy_storage['obstacle_masks'][file_idx]
        file_length = int(npy_storage['file_lengths'][file_idx])

        if mask_array.ndim != 3:
            raise ValueError(
                f"Expected obstacle mask shape (N, H, W), got {mask_array.shape}"
            )
        if mask_array.shape[0] != file_length:
            raise ValueError(
                f"Obstacle mask sample count {mask_array.shape[0]} does not match data sample count {file_length}"
            )

        mask = np.asarray(mask_array[local_idx], dtype=np.float32)
        return mask

    def _load_raw_sample(self, dataset_idx, data_idx):
        if self.storage_formats[dataset_idx] == 'npy':
            return self._load_npy_sample(self.data_files[dataset_idx], data_idx)
        if callable(self.data_files[dataset_idx]):
            return self.data_files[dataset_idx](data_idx)[:]
        return self.data_files[dataset_idx]['data'][data_idx][:]

    def _load_obstacle_mask(self, dataset_idx, data_idx):
        """Load obstacle mask for dataset. Returns None if mask not available."""
        if self.storage_formats[dataset_idx] == 'npy':
            return self._load_npy_obstacle_mask(self.data_files[dataset_idx], data_idx)
        return None
    ...
    def pad_obstacle_mask(self, mask):
        '''
        Pad obstacle mask to unified spatial resolution
        :param mask: (H, W) or (H, W, T) - typically obstacle mask is spatial only
        :return: (res, res) tensor
        '''
        mask_tensor = torch.from_numpy(mask).float()
        
        if mask_tensor.ndim == 3:  # (H, W, T) - take first timestep
            mask_tensor = mask_tensor[..., 0]
        
        # Now mask_tensor is (H, W)
        H, W = mask_tensor.shape
        # Reshape for interpolation: (1, 1, H, W)
        mask_tensor = mask_tensor.unsqueeze(0).unsqueeze(0)
        mask_tensor = F.interpolate(mask_tensor, size=(self.res, self.res), mode='nearest').squeeze(0).squeeze(0)
        
        return mask_tensor

    def get_target_mask(self, x, size_orig, obstacle_mask=None):
        '''
        :param x: single data, H, W, T, C
        :param size_orig: original size of x
        :param obstacle_mask: (res, res) tensor with obstacle mask (0 for obstacle, 1 for fluid)
        ...
        # Apply obstacle mask if provided
        if obstacle_mask is not None:
            # Broadcast obstacle mask to all channels
            obstacle_mask_expanded = obstacle_mask.unsqueeze(-1).unsqueeze(-1)  # (res, res, 1, 1)
            msk = msk * obstacle_mask_expanded
    ...
    def __getitem__(self, idx):
        ...
        data_idx = int(data_idx // self.data_weights[dataset_idx])
        # t_0 = time.time()
        sample = torch.from_numpy(self._load_raw_sample(dataset_idx, data_idx)).float()
        ...
        if sample.shape[-2] <= self.t_in:
            raise ValueError(
                f"Dataset {self.data_names[dataset_idx]} has total time steps {sample.shape[-2]} <= t_in {self.t_in}. "
                f"Please reduce t_in or use longer trajectories."
            )

        # Load obstacle mask if available
        obstacle_mask_raw = self._load_obstacle_mask(dataset_idx, data_idx)
        obstacle_mask = None
        if obstacle_mask_raw is not None:
            obstacle_mask = self.pad_obstacle_mask(obstacle_mask_raw)
        ...
        if self.train:  ## sample [0, t_in] and [t_in, t_in+ t_ar] for training ,trucated if too long
            ...
            msk = self.get_target_mask(sample, orig_size, obstacle_mask=obstacle_mask)

        if self.normalize and self.normalizers[int(dataset_idx)] is not None:
        ...
        if self.downsamples[dataset_idx] != (1, 1):
            ...
            if obstacle_mask is not None:
                obstacle_mask = obstacle_mask[::self.downsamples[dataset_idx][0],::self.downsamples[dataset_idx][1]]

        idx_cls = torch.LongTensor([dataset_idx])   #TODO: now return relative idx in given datasets, finally we need global idx
...
class MixedMaskedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:  ## sample [0, t_in] and [t_in, t_in+ t_ar] for training ,trucated if too long
            ...
            target_msk = self.get_target_mask(sample, orig_size, obstacle_mask=obstacle_mask)
